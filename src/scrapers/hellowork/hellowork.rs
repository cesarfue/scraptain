...
impl HelloWorkScraper {
    ...
    async fn fetch_job_details(
        ...
        if html.contains("hellowork.com/signup") || html.contains("authwall") {
            return None;
        }
...
impl PlatformScraper for HelloWorkScraper {
    async fn search(&self, params: JobSearchParams) -> Result<Vec<Job>> {
        ...
        while all_jobs.len() < results_wanted && start < max_start {
            ...
            println!("Found card selector {:?}", job_card_selector);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_hellowork_scraper() {
        ...
        let scraper = HelloWorkScraper::new(client);
